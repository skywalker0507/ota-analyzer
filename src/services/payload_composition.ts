...
import { toNumericSize } from '../utils/format'
 ...
 * @param {Number} value
 */
function addNumberToMap(map: Map<string, number>, key: string, value: unknown) {
  const numericValue = toNumericSize(value)
  if (!map.get(key)) {
    map.set(key, 0)
  }
...
export function operatedBlockStatistics(
  ...
  for (let partition of partitions) {
    for (let operation of partition.operations!) {
      let operationType = String(opType.mapType.getWithDefault(operation.type))
      addNumberToMap(
        operatedBlocks,
        operationType,
...
function partitionPayloadSize(
  ...
  for (let op of partition.operations!) {
    size += toNumericSize(op.dataLength)
  }
  return size
}
...
export function mergeOperationStatistics(
  ...
  for (let partition of partitions) {
    ...
    totalBlocks += Math.ceil(toNumericSize(partition.newPartitionInfo.size) / blockSize)
  }
...
export function numBlocks(
  ...
  const accumulator = (total: number, ext: chromeos_update_engine.IExtent) =>
    total + toNumericSize(ext.numBlocks)
...
export function distributeFilenames(
  ...
  for (let i = 0; i < filenames.length; i++) {
    addNumberToMap(
      distributedLengths,
      filenames[i],
      Math.round((toNumericSize(length) * toNumericSize(exts[i].numBlocks)) / totalBlocks)
    )
  }
