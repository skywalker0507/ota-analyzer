...
class OTAPayloadBlobWriter extends Writer {
  ...
  /**
   * A zip.Writer that reads only the metadata prefix of payload.bin.
   * Uses AbortController to stop zip.js from reading the full payload blob.
   */
  constructor(contentType = '', abortController: AbortController | null = null) {
    super()
    this.offset = 0
  ...
  isComplete() {
    return this.prefixLength > 0 && this.offset >= this.prefixLength
  }
  ...
  async writeUint8Array(array: Uint8Array) {
    ...
    this.blob = new Blob([this.blob, array], { type: this.contentType })
  ...
  async unzip() {
    ...
    for (let entry of entries) {
      if (entry.filename == 'payload.bin') {
        ...
        const writer = new OTAPayloadBlobWriter('', abortController)
        try {
